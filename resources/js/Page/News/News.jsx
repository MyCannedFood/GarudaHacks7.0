            ...
            <div className="max-w-[1400px] mx-auto w-full px-6 md:px-12 py-8 flex-grow">
                <form onSubmit={handleSearch} className="flex flex-wrap items-center gap-4 mb-8">
                    ...
                    <div className="relative">
                        <select
                            ...
                            className="border border-gray-300 bg-white text-sm py-2 pl-4 pr-10 outline-none appearance-none rounded-md w-full md:w-48 cursor-pointer"
                        >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
                    {loading && Array.from({ length: 4 }).map((_, i) => (
                        <div key={i} className="border border-gray-200 bg-white flex flex-col rounded-none shadow-sm">
                            ...
                            <div className="p-5 flex flex-col flex-grow">
                                ...
                                <div className="h-3 bg-gray-200 rounded animate-pulse w-1/3 mt-auto" />
                            </div>
                        </div>
                    ))}
                    ...
                    {pageItems.map((item) => (
                        <div key={item.id} className="border border-gray-200 bg-white flex flex-col rounded-none shadow-sm">
                ...
                <div className="flex justify-center items-center space-x-2">
                    <button
                        ...
                        className={`p-2 ${activePage === 1 ? 'text-gray-300 cursor-not-allowed' : 'text-black hover:text-gray-700'} transition-colors`}
                    >
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                        <button
                            ...
                            className={`w-8 h-8 flex items-center justify-center text-sm font-medium rounded-sm transition-colors ${activePage === page ? 'bg-[#0ea5e9] text-white' : 'text-gray-500 hover:bg-gray-100'}`}
                        >
                            {page}
                        </button>
                    ))}
                    <button
                        ...
                        className={`p-2 ${activePage === totalPages ? 'text-gray-300 cursor-not-allowed' : 'text-black hover:text-gray-700'} transition-colors`}
                    >
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
